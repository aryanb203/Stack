...
string InfixToPrefix(string k){

    stack<char> st;
    string res=""; //resultant string
    string s="";
    for(int j=0, l=k.length()-1; l>=0; l--, j++){   //reversing the expression and changing the brackets
        if(k[l]=='(')
            s+=')';
        else if(k[l]==')')
            s+='(';
   ...
   for(int i=0; i<s.length(); i++){
    ...
    reverse(res.begin(), res.end()); //reversing the string again at the end
    return res;
}
